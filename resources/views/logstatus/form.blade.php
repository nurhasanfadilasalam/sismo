...
@section('title', '| Penjualan Partner')
@section("content")
...
<section class="section">
    <div class="section-header">
        <h1>Penjualan Partner</h1>
    </div>
    <div class="section-body">
        <form id="salesForm" action="{{route('sales.store')}}" method="POST" enctype="multipart/form-data" class="bg-white shadow-sm p-3">
            @csrf
            <div class="row">
                @if (session('status'))
                    <div class="col-md-12">
        ...
        $.each(invenSelected, (k, v) => {
            ...
            $('#listPicking').append(
                `<li class="list-group-item d-flex justify-content-between align-items-center" id="liConList-${v.id}">
                    ...
                    <span class="float-right">(${v.qty} ${v.unit}) <b>${convertToRp(v.subTotal)}</b> <button type="button" data-id="${v.id}" class="btn btn-icon btn-sm btn-danger btnDeleteList"><i class="fa fa-trash"></i></button></span>
                </li>`
            )
        })
    }
